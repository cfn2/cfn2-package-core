...
function makeResourceTasks(options) {
  ...
  return Object.entries(options.Resources).map(([logicalId, resource]) => callback => {
    ...
    readFile(artifactPath, (err, content) => {
      if (!err) {
        return packFile(artifactPath, { content }, (err, file, thumbprint) => {
          if (err) {
            return callback(err);
          }

          return upload(file, thumbprint);
        });
      }
      ...
      s3.headObject(params, (err, data) => {
        if (err) {
          ...
          if (statusCode !== 200 && statusCode !== 404) {
            return callback(new Error(`Response ${statusCode} for ${s3url}`));
          }
        }
...
function packFile(path, options, callback) {
  const {
    content,
  } = options;
  ...
  }).then(data => callback(null, data, md5(content)))
